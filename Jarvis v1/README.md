...
- Generates conversational responses through a local Ollama model (`nous-hermes2`).
...
At startup, Jarvis asks `Voice mode? (y/n):`
- Choose `y` for microphone + wake phrase mode (`hey jarvis`).
- Choose `n` for text command mode.
